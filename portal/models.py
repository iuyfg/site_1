...
from django.db import models
from django.contrib.auth.models import User
# Импортируем хранилище Cloudinary
...
class UserProfile(models.Model):
    ...
    # Явно указываем Cloudinary для фото профиля
    profile_photo = models.ImageField(
        upload_to='profile_photos/',
        storage=MediaCloudinaryStorage(),  # <-- ВАЖНО
        null=True,
        blank=True,
...
class Product(models.Model):
    ...
    # Явно указываем Cloudinary для всех изображений товара
    image = models.ImageField(
        upload_to='products/',
        storage=MediaCloudinaryStorage(),  # <-- ВАЖНО
        verbose_name='Изображение'
...
class Promotion(models.Model):
    # Явно указываем Cloudinary для изображения акции
    image = models.ImageField(
        upload_to='promotions/',
        storage=MediaCloudinaryStorage(),  # <-- ВАЖНО
        verbose_name='Изображение'
...
class Review(models.Model):
    ...
    def __str__(self):
        # Проверка на наличие профиля, чтобы избежать ошибки, если профиль не создан
        if hasattr(self.user, 'userprofile'):
            return f'Отзыв от {self.user.userprofile.first_name} {self.user.userprofile.last_name}'
        return f'Отзыв от пользователя {self.user.username}'
